...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    set -eux; \
    mkdir -p /out/bin; \
    for cmd in academic-api announcement-api apply-table-privileges build-class-change-notifications-job dispatch-notifications-job insert-faculty-rooms migrate scrape-class-changes-job user-api; do \
